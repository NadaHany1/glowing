...
export default function EcommerceApp() {
  ...
  return (
    <div className="min-h-screen bg-white font-urbanist">
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6">
              <button onClick={() => setIsLoginOpen(true)} className="hover:text-gray-600">
          ...
          <nav className="hidden lg:flex justify-center gap-12 mt-6 pt-4 border-t">
            {['Home', 'Collection', 'Shop', 'Offer', 'Blog'].map(item => (
              <a key={item} href={`#${item.toLowerCase()}`} className="font-semibold hover:text-green-700 transition">
                {item}
              </a>
            ))}
          </nav>
      ...
      {isNavOpen && (
        <>
          <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setIsNavOpen(false)} />
          <div className="fixed left-0 top-0 bottom-0 w-80 bg-white z-50 p-8">
            ...
            <nav className="flex flex-col gap-4">
              {['Home', 'Collection', 'Shop', 'Offer', 'Blog', 'Cart'].map(item => (
                <a key={item} href={`#${item.toLowerCase()}`} className="font-semibold text-sm uppercase tracking-wide hover:text-green-700" onClick={() => setIsNavOpen(false)}>
                  {item}
                </a>
              ))}
            </nav>
      ...
      {isCartOpen && (
        <>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-96 bg-gray-100 z-50 p-6 overflow-y-auto">
            <button onClick={() => setIsCartOpen(false)} className="absolute top-4 right-4 text-3xl hover:text-green-700">
              <X />
            </button>
            <h2 className="text-3xl font-semibold text-center mt-8 mb-6">YOUR CART</h2>
            
            {cart.map(item => (
              <div key={item.id} className="grid grid-cols-[32%_50%_18%] gap-4 items-center mb-4 bg-white p-3 rounded">
                <Image fill src={item.image} alt={item.name} className="w-full h-24 object-contain" />
                <div>
                  <h3 className="text-sm font-semibold uppercase mb-2">{item.name}</h3>
                  <p className="font-semibold mb-2">${item.price.toFixed(2)}</p>
                  <input
                    type="number"
                    value={item.quantity}
                    onChange={(e) => updateQuantity(item.id, parseInt(e.target.value))}
                    className="w-16 text-center border border-green-700 rounded p-1"
                    min="1"
                  />
                </div>
                <button onClick={() => removeFromCart(item.id)} className="text-xl hover:text-green-700">
                  <X />
                </button>
              </div>
            ))}
            ...
            {cart.length > 0 ? (
              <>
                <div className="flex justify-end items-center gap-2 mt-6 pt-4 border-t border-gray-400">
                  <span className="text-2xl font-semibold">TOTAL:</span>
                  <span className="text-2xl">${getTotal()}</span>
                </div>
      ...
      {/* Login Modal */}
      {isLoginOpen && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-20" onClick={() => setIsLoginOpen(false)}>
          <div className="bg-white rounded-2xl p-8 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold">Login</h2>
              <button onClick={() => setIsLoginOpen(false)} className="text-3xl text-gray-400 hover:text-black">×</button>
            </div>
            <form className="space-y-4">
              <div>
                <label className="block mb-1 font-medium">Email:</label>
                <input type="email" required className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-green-700 outline-none" />
              </div>
              <div>
                <label className="block mb-1 font-medium">Phone number:</label>
                <input type="text" required className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-green-700 outline-none" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="password" required className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-green-700 outline-none" />
              </div>
              <button type="submit" className="w-full border-2 border-green-700 text-green-700 font-semibold py-2 rounded-lg hover:bg-green-700 hover:text-white transition">
                Submit
              </button>
            </form>
          </div>
        ...
        <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
          {heroSlides.map((slide, index) => (
            <div key={index} className="relative min-w-full h-[80vh]">
              <Image fill  src={slide} alt={`Slide ${index + 1}`} className="w-full h-full object-cover" />
              <div className="absolute bottom-[20%] left-8 text-left max-w-lg">
                <h1 className="text-5xl md:text-6xl font-semibold mb-4 leading-tight">
                  Reveal The<br />Beauty of Skin
                </h1>
                <p className="text-lg mb-4">Made using clean, non-toxic ingredients, our products are designed for everyone.</p>
                <p className="text-xl font-semibold mb-6">Starting at $7.99</p>
                <button className="bg-black text-white px-8 py-3 rounded hover:bg-green-700 transition">Shop Now</button>
              </div>
            </div>
          ))}
        </div>
        <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl p-3 hover:bg-black/30 transition">
          <ChevronLeft />
        </button>
        <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 text-2xl p-3 hover:bg-black/30 transition">
          <ChevronRight />
        </button>
      </section>
      ...
      <section id="collection" className="py-16">
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-gray-200 min-h-[400px] p-10 flex flex-col justify-end bg-cover bg-center" style={{backgroundImage: "url('/images/collection-1.jpg')"}}>
              <h2 className="text-3xl font-semibold mb-3">Summer Collection</h2>
              <p className="mb-6">Starting at $17.99</p>
              <a href="#" className="flex items-center gap-2 font-semibold hover:text-green-700">Shop Now →</a>
            </div>
            <div className="bg-gray-200 min-h-[400px] p-10 flex flex-col justify-end bg-cover bg-center" style={{backgroundImage: "url('/images/collection-2.jpg')"}}>
              <h2 className="text-3xl font-semibold mb-3">Explore What is New?</h2>
              <p className="mb-6">Get the glow</p>
              <a href="#" className="flex items-center gap-2 font-semibold hover:text-green-700">Discover Now →</a>
            </div>
            <div className="bg-gray-200 min-h-[400px] p-10 flex flex-col justify-end bg-cover bg-center" style={{backgroundImage: "url('/images/collection-3.jpg')"}}>
  ...
  <div className="mx-auto px-4">
    <div className="flex justify-between items-center mb-12">
      <h2 className="text-4xl font-semibold">Our Bestsellers</h2>
      <a href="#" className="flex items-center gap-2 font-semibold hover:text-green-700">
        Shop All Products →
      </a>
    </div>
        ...
        >
          ...
          <div className="relative w-full h-96">
            <Image
              ...
              sizes="(max-width: 1024px) 100vw, 25vw" // optional for better optimization
            />

            {/* Badge */}
            {product.badge && (
              <span className="absolute top-4 left-4 bg-green-700 text-white px-3 py-1 text-sm font-bold rounded">
                {product.badge}
              </span>
            )}

            {/* Action buttons */}
            <div className="absolute top-1/2 right-0 -translate-y-1/2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 group-hover:-translate-x-5 transition duration-300">
              <button
          ...
          <div className="p-4 text-center">
            ...
            <h3 className="font-medium mb-3 hover:underline hover:text-green-700 transition">
              {product.name}
            </h3>
            <div className="flex justify-center items-center gap-2">
              ...
              <span className="text-sm text-gray-600">5170 reviews</span>
            </div>
          </div>
        </div>
      ))}
...
<section className="py-16">
  <div className="container mx-auto px-4">
    <div className="flex justify-between items-center mb-12">
      <h2 className="text-4xl font-semibold">Under $75</h2>
      <a href="#" className="flex items-center gap-2 font-semibold hover:text-green-700">
        Shop All Products →
      </a>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-4 gap-12 mb-16">
            <div>
              ...
              <p className="mb-2">Find a location nearest you. See <a href="#" className="underline">Our Stores</a></p>
              <p className="font-bold my-3">+391 (0)35 2568 4593</p>
              <p>hello@domain.com</p>
            </div>
            <div>
              <h3 className="text-xl font-semibold mb-4">Useful links</h3>
              <div className="space-y-2">
                <a href="#" className="block hover:underline">New Products</a>
                <a href="#" className="block hover:underline">Best Sellers</a>
                <a href="#" className="block hover:underline">Bundle & Save</a>
                <a href="#" className="block hover:underline">Online Gift Card</a>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-semibold mb-4">Information</h3>
              <div className="space-y-2">
                <a href="#" className="block hover:underline">Start a Return</a>
                <a href="#" className="block hover:underline">Contact Us</a>
                <a href="#" className="block hover:underline">Shipping FAQ</a>
                <a href="#" className="block hover:underline">Terms & Conditions</a>
                <a href="#" className="block hover:underline">Privacy Policy</a>
              </div>
            </div>
            <div>
              <h3 className="text-2xl font-semibold mb-4">Good emails.</h3>
              <p className="mb-6 text-sm">Enter your email below to be the first to know about new collections and product launches.</p>
              <form className="space-y-3">
                ...
                <button type="submit" className="w-full bg-black text-white py-2 rounded hover:bg-green-700 transition">Subscribe</button>
              </form>
            </div>
          </div>
      ...
      {showBackToTop && (
        <button
          ...
          className="fixed bottom-8 right-8 bg-white text-green-700 p-4 rounded-full shadow-lg hover:bg-green-700 hover:text-white transition z-50"
        >
